/**
 * PSD Layer Splitter - Browse buttons for psd_path and output_dir.
 *
 * Reuses the FolderBrowser from image_folder_cowboy.js via
 * the shared /trent/browse and /trent/validate_path endpoints.
 */

import { app } from "../../scripts/app.js";
import { api } from "../../scripts/api.js";

/* ---- Shared CSS (idempotent injection) ---- */
...
class PathBrowser {
    constructor(initialPath, onSelect, opts = {}) {
        this.currentPath = initialPath || "";
        this.onSelect = onSelect;
        this.dirsOnly = opts.dirsOnly ?? true;
        this.extensions = opts.extensions || "";
        this.label = opts.label || "Path:";
        this.selectLabel = opts.selectLabel || "Select";
        this.dialog = null;
        this.items = [];
    }
        ...
        this.dialog.innerHTML = `
            ...
            <div class="browser-buttons">
                ...
                <button class="browser-btn primary"
                        data-action="select">${escapeHtml(this.selectLabel)}</button>
            </div>
        `;
    ...
    async loadPath(path) {
        try {
            ...
            if (this.extensions) {
                params.set("extensions", this.extensions);
            }
            if (this.dirsOnly) {
                params.set("dirs_only", "true");
            }
...
/* ---- Helper to add a browse button after a widget ---- */

function addBrowseButton(node, widgetName, browserOpts) {
    ...
    // Move button right after target widget
...
app.registerExtension({
    name: "Trent.PSDLayerSplitter",

    async beforeRegisterNodeDef(nodeType, nodeData, _app) {
        if (nodeData.name !== "PSDLayerSplitter") return;

        const orig = nodeType.prototype.onNodeCreated;
        nodeType.prototype.onNodeCreated = function () {
            const result = orig?.apply(this, arguments);

            addBrowseButton(this, "psd_path", {
                dirsOnly: false,
                extensions: "psd",
                label: "PSD File:",
                selectLabel: "Select PSD",
            });

            addBrowseButton(this, "output_dir", {
                dirsOnly: true,
                label: "Output Folder:",
...
console.log("[TrentNodes] PSDLayerSplitter extension loaded");
