...
import { isEnabled, setEnabled, setNativeRibbons } from "./enabled.js";
...
const RIBBON_STYLE_SETTING = "cablemanagement.nativeRibbons";
...
app.registerExtension({
  // Renamed from upstream's "cablemanagement" ONLY so this copy is identifiable in the
  // extension list and does not hard-collide if the original folder is ever re-enabled.
  // Setting ids and the graph.extra comb key are deliberately left ALONE: they carry your
  // existing preferences and every workflow you have already rigged.
  name: "TrentNodes.CableManagement",
  ...
  settings: [
    ...
    {
      id: RIBBON_STYLE_SETTING,
      // Same section as the master toggle -- it changes how ribbons draw.
      category: ["LiteGraph", "Graph", "Cable Management Ribbon Style"],
      name: "Ribbon runs match the link style",
      tooltip:
        "Outside PCB mode, draw each ribbon lane with the regular link rendering " +
        "(spline/linear/straight) between its gates instead of a routed trace. " +
        "PCB mode always draws traces.",
      type: "boolean",
      defaultValue: false,
      onChange: (value) => {
        setNativeRibbons(value === true);
        app.graph?.setDirtyCanvas?.(true, true);
      },
    },
    {
  ...
  async setup() {
    ...
    setNativeRibbons(app.extensionManager?.setting?.get(RIBBON_STYLE_SETTING) === true);
