...
// The outward ribbon faces of a comb's two gates, in DRAWN SEGMENT order --
// combCrossing only matches in-tooth -> out-tooth, so start is always the in gate.
// Same rule gateGeom uses; separate because the native ribbon style needs the
// directions without any of the geometry.
export function crossingDirs(comb) {
  return {
    sDir: comb.in.pins === 'left' ? 'right' : 'left',
    eDir: comb.out.pins === 'left' ? 'right' : 'left'
  }
}
