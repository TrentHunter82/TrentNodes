...
import { combCrossing, combPass, crossingDirs, crossingPts, gateRects, toothOf } from './combs.js'
...
export function routeFor(canvas, linkData, pcb = true, nativeRibbons = false) {
  ...
  if (!sIsPin || !eIsPin) {
    ...
    if (cross) {
      // Native ribbon style (non-PCB modes only): no trace, no cache entry. The
      // caller re-aims the segment off the gate faces and lets core draw it.
      if (!pcb && nativeRibbons) return { comb: true, native: true, ...crossingDirs(cross.comb) }
