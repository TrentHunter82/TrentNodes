...
import { isEnabled, nativeRibbonsOn } from "./enabled.js";

const state = { patched: false, option: false, PCB: 3 };

// Teeth are reroutes, so core hands a ribbon segment CENTER at both ends and control
// points tangent to the reroute CHAIN (Reroute.calculateAngle). Through a stack of
// teeth that tangent runs along the pin column, so the lane leaves its gate sideways
// and lolls into an S -- nothing like the wire a node pin gets. Re-aim the segment at
// the gates' outward ribbon faces and hand core the control points it computes for a
// node pin (LinkRenderer.renderLinkDirect: quarter of the endpoint distance, no floor),
// so arrows and flow dots -- which re-derive that same quarter from the DIRECTIONS in
// computeConnectionPoint -- land exactly on the drawn curve. The linear and straight
// builders read only the directions, so they come out right from the same two writes.
const DIR_X = { left: -1, right: 1 };
const DIR_Y = { up: -1, down: 1 };

function aimNativeRibbon(linkData, r) {
  linkData.startDirection = r.sDir;
  linkData.endDirection = r.eDir;
  const s = linkData.startPoint, e = linkData.endPoint;
  const d = Math.hypot(e.x - s.x, e.y - s.y) * 0.25;
  linkData.controlPoints = [
    { x: s.x + (DIR_X[r.sDir] ?? 0) * d, y: s.y + (DIR_Y[r.sDir] ?? 0) * d },
    { x: e.x + (DIR_X[r.eDir] ?? 0) * d, y: e.y + (DIR_Y[r.eDir] ?? 0) * d }
  ];
}
...
function combsOn() {
  // RIBBON MACHINERY -- combPass, gates, gestures, the gate->gate run -- runs in
  // EVERY link render mode (mixed-modes ruling: ribbons always on; outer segments
  // follow the native mode). How the internal run DRAWS is the separate ribbon
  // style choice: PCB-drawn by default, native with nativeRibbonsOn().
  ...
  prProto.drawLink = function (ctx, linkData, context) {
    if (combsOn() && linkData.id !== "temp" && linkData.id !== "dragging") {
      ...
      try {
        ...
        // gate->gate run stays a trace (or, with native ribbon style, keeps core's
        // geometry), everything else renders natively.
        const r = routeFor(app.canvas, linkData, active(), nativeRibbonsOn());
        if (r) {
          if (r.native) aimNativeRibbon(linkData, r);
          else linkData.__route = r;
