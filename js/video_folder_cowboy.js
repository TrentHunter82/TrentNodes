/**
 * Video Folder Cowboy - Folder browser widget
 *
 * Adds a browse button next to the directory input.
 * Reuses FolderBrowser from image_folder_cowboy.
 */

import { app } from "../../scripts/app.js";
import { api } from "../../scripts/api.js";

// CSS for the folder browser dialog (shared ID avoids duplication)
const BROWSER_CSS = `
...
// Inject CSS once (check if already injected by image_folder_cowboy)
function injectCSS() {
    if (document.getElementById("trent-folder-browser-css")) return;
...
// Folder browser dialog (video-aware)
class VideoFolderBrowser {
    ...
    async loadPath(path) {
        try {
            ...
            console.error("[VideoFolderCowboy] Failed to load path:", err);
            this.items = [];
            this.renderList();
    ...
    async validatePath(path) {
        ...
        try {
            const params = new URLSearchParams({ path, type: "dir", media: "video" });
            const response = await api.fetchApi("/trent/validate_path?" + params);
            const result = await response.json();

            if (result.valid && result.type === "dir") {
                this.status.textContent = `Valid folder - ${result.count} video(s) found`;
    ...
    renderList() {
        ...
        for (const item of this.items) {
            ...
            const icon = isDir ? "[D]" : (isVideo ? "[V]" : "[F]");
...
app.registerExtension({
    name: "Trent.VideoFolderCowboy",

    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name !== "VideoFolderCowboy") {
            return;
        }

        console.log("[TrentNodes] Registering VideoFolderCowboy browse button");
        ...
        nodeType.prototype.onNodeCreated = function() {
            ...
            if (!dirWidget) {
                console.warn("[VideoFolderCowboy] Could not find directory widget");
                return result;
            }

            const node = this;
            const browseWidget = this.addWidget("button", "browse_folder", "Browse...", function() {
                const browser = new VideoFolderBrowser(dirWidget.value, (selectedPath) => {
...
console.log("[TrentNodes] VideoFolderCowboy extension loaded");
