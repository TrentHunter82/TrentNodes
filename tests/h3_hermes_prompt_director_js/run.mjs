// Checks the sibling Hermes node visibility extension against a mocked frontend.
//
//   node tests/h3_hermes_prompt_director_js/run.mjs
//
...
const source = join(here, "..", "..", "js", "h3_hermes_prompt_director.js");

const stage = mkdtempSync(join(tmpdir(), "trentnodes-h3h-"));
...
function makeNode(values = {}, wired = []) {
    ...
    add("quality_mode", "balanced");
    add("research_policy", "when_uncertain");
    add("fps", 24.0);
    ...
    add("visible_text", "");
    add("hermes_base_url", "http://127.0.0.1:8642");
    add("timeout_seconds", 900);
    add("poll_interval_seconds", 1.0);
    add("cleanup_policy", "delete_on_success");
    add("hermes_provider", "");
    add("hermes_model", "");
    ...
    const node = {
        constructor: { comfyClass: "TrentH3HermesPromptDirector" },
        widgets,
        inputs,
        size: [400, 600],
        computeSize: () => [400, 600],
...
console.log(
    failures
        ? `\n${failures} failure(s)\n`
        : "\nAll H3 Hermes Prompt Director JS tests passed.\n",
);
