    ...
    def forward(
        ...
        *,
        ...
        if any(self.add_pe_to_qkv[1:]):
            ...
            k = (
                mem_with_pe
                if self.add_pe_to_qkv[1]
                else mem
            )
            v = (
                mem_with_pe
                if self.add_pe_to_qkv[2]
                else mem
            )
        else:
            k = v = mem
...
class PixelFFN(nn.Module):
    ...
    def forward(
        ...
        pixel_flat = pixel_flat.view(
            bs * num_objects, h, w, self.dim
        )
        pixel_flat = pixel_flat.permute(
            0, 3, 1, 2
        ).contiguous()
...
def _get_activation_fn(
    ...
    raise RuntimeError(
        "activation should be relu/gelu,"
        f" not {activation}"
    )
