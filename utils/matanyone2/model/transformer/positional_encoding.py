...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        sin_inp_y = torch.einsum(
            "i,j->ij", pos_y, self.inv_freq
        )
        sin_inp_x = torch.einsum(
            "i,j->ij", pos_x, self.inv_freq
        )
        ...
        if not self.channel_last and self.transpose_output:
            pass
        elif (not self.channel_last) or (
            self.transpose_output
        ):
            emb = emb.permute(2, 0, 1)

        self.cached_penc = (
            emb.unsqueeze(0).repeat(batch_size, 1, 1, 1)
        )
        if num_objects is None:
            return self.cached_penc
        else:
