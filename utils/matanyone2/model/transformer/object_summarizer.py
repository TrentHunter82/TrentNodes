...
def _weighted_pooling(
    ...
    area = (
        weights.flatten(start_dim=2, end_dim=3)
        .sum(2)
        .unsqueeze(-1)
    )
    return sums, area
...
class ObjectSummarizer(nn.Module):
    def __init__(self, model_cfg: DictConfig):
        ...
        self.weights_pred = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Linear(
                self.embed_dim, self.num_summaries
            ),
        )
    ...
    def forward(
        ...
        with safe_autocast(enabled=False):
            ...
            sums, area = _weighted_pooling(
                repeated_masks, feature, logits
            )
