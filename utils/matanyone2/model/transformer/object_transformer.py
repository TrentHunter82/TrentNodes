...
class QueryTransformerBlock(nn.Module):
    def __init__(self, model_cfg: DictConfig):
        ...
        self.read_from_pixel = CrossAttention(
            ...
            self.num_heads,
            add_pe_to_qkv=(
                this_cfg.read_from_pixel.add_pe_to_qkv
            ),
        )
        self.self_attn = SelfAttention(
            ...
            self.num_heads,
            add_pe_to_qkv=(
                this_cfg.query_self_attention.add_pe_to_qkv
            ),
        )
        ...
        self.read_from_query = CrossAttention(
            ...
            self.num_heads,
            add_pe_to_qkv=(
                this_cfg.read_from_query.add_pe_to_qkv
            ),
...
class QueryTransformer(nn.Module):
    def __init__(self, model_cfg: DictConfig):
        ...
        self.summary_to_query_init = nn.Linear(
            self.embed_dim, self.embed_dim
        )
        self.summary_to_query_emb = nn.Linear(
            self.embed_dim, self.embed_dim
        )

        self.pixel_pe_scale = model_cfg.pixel_pe_scale
        self.pixel_pe_temperature = (
            model_cfg.pixel_pe_temperature
        )
        ...
        self.blocks = nn.ModuleList(
            QueryTransformerBlock(model_cfg)
            for _ in range(self.num_blocks)
        )
        self.mask_pred = nn.ModuleList(
            nn.Sequential(
                nn.ReLU(),
                GConv2d(self.embed_dim, 1, kernel_size=1),
            )
    ...
    def forward(
        ...
        query = (
            self.query_init.weight.unsqueeze(0).expand(
                bs * num_objects, -1, -1
            )
            + obj_init
        )
        query_emb = (
            self.query_emb.weight.unsqueeze(0).expand(
                bs * num_objects, -1, -1
            )
            + obj_emb
        )
        ...
        pixel_pe = self.spatial_pe(
            pixel.flatten(0, 1)
        )
        pixel_emb = (
            pixel_emb.flatten(3, 4)
            .flatten(0, 1)
            .transpose(1, 2)
            .contiguous()
        )
        ...
        attn_mask = self._get_aux_mask(
            aux_logits, selector, seg_pass=seg_pass
        )
        aux_features['logits'].append(aux_logits)
        for i in range(self.num_blocks):
            query, pixel, q_weights, p_weights = (
                self.blocks[i](
                    query,
                    pixel,
                    query_emb,
    ...
    def _get_aux_mask(
        ...
        is_foreground = (
            logits[:, 1:]
            >= logits.max(dim=1, keepdim=True)[0]
        )
        foreground_mask = is_foreground.bool().flatten(
            start_dim=2
        )
        ...
        aux_mask = torch.cat(
            [aux_foreground_mask, aux_background_mask],
            dim=1,
        )

        aux_mask[
            torch.where(
                aux_mask.sum(-1) == aux_mask.shape[-1]
            )
        ] = False
