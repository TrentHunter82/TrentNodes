"""
big_modules.py - Higher-level network blocks.

x - features shared between objects
g - features not shared (batch_size * num_objects * ...)
"""
...
class PixelEncoder(nn.Module):
    def __init__(self, model_cfg: DictConfig):
        ...
        self.is_resnet = (
            'resnet' in model_cfg.pixel_encoder.type
        )
        is_pretrained_resnet = getattr(
            model_cfg, "pretrained_resnet", True
        )
        if self.is_resnet:
            if model_cfg.pixel_encoder.type == 'resnet18':
                network = resnet.resnet18(
                    pretrained=is_pretrained_resnet
                )
...
class KeyProjection(nn.Module):
    def __init__(self, model_cfg: DictConfig):
        ...
        self.pix_feat_proj = nn.Conv2d(
            in_dim, mid_dim, kernel_size=1
        )
        self.key_proj = nn.Conv2d(
            mid_dim, key_dim, kernel_size=3, padding=1
        )
        self.d_proj = nn.Conv2d(
            mid_dim, 1, kernel_size=3, padding=1
        )
        self.e_proj = nn.Conv2d(
            mid_dim, key_dim, kernel_size=3, padding=1
        )
    ...
    def forward(
        ...
        *,
        ...
        shrinkage = (
            self.d_proj(x) ** 2 + 1 if need_s else None
        )
        selection = (
            torch.sigmoid(self.e_proj(x))
            if need_e
            else None
        )
...
class MaskEncoder(nn.Module):
    def __init__(
        ...
        self.fuser = GroupFeatureFusionBlock(
            pixel_dim, final_dim, value_dim
        )

        self.sensory_update = SensoryDeepUpdater(
            value_dim, sensory_dim
        )
    ...
    def forward(
        ...
        *,
        ...
        for i in range(0, num_objects, chunk_size):
            ...
            if deep_update:
                if fast_path:
                    new_sensory = self.sensory_update(
                        g_chunk, sensory
                    )
                else:
                    new_sensory[
...
class MaskDecoder(nn.Module):
    def __init__(self, model_cfg: DictConfig):
        ...
        self.up_16_8 = MaskUpsampleBlock(
            up_dims[0], up_dims[1]
        )
        self.up_8_4 = MaskUpsampleBlock(
            up_dims[1], up_dims[2]
        )
        self.up_4_2 = MaskUpsampleBlock(
            up_dims[2], up_dims[3]
        )
        self.up_2_1 = MaskUpsampleBlock(
            up_dims[3], up_dims[4]
        )

        self.pred_seg = nn.Conv2d(
            up_dims[-1], 1, kernel_size=3, padding=1
        )
        self.pred_mat = nn.Conv2d(
            up_dims[-1], 1, kernel_size=3, padding=1
        )
    ...
    def forward(
        ...
        *,
        ...
        f8, f4, f2, f1 = self.decoder_feat_proc(
            ms_image_feat[1:]
        )
        if chunk_size < 1 or chunk_size >= num_objects:
            chunk_size = num_objects
        ...
        logits = logits.view(
            batch_size, num_objects, *logits.shape[-2:]
        )
