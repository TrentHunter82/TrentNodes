...
class LinearPredictor(nn.Module):
    def __init__(self, x_dim: int, pix_dim: int):
        super().__init__()
        self.projection = GConv2d(
            x_dim, pix_dim + 1, kernel_size=1
        )
    ...
    def forward(
        ...
        pix_feat = pix_feat.unsqueeze(1).expand(
            -1, num_objects, -1, -1, -1
        )
        logits = (
            (pix_feat * x[:, :, :-1]).sum(dim=2)
            + x[:, :, -1]
        )
        return logits
...
class AuxComputer(nn.Module):
    def __init__(self, cfg: DictConfig):
        ...
        self.use_query_aux = (
            cfg.model.aux_loss.query.enabled
        )
        ...
        if use_sensory_aux:
            self.sensory_aux = LinearPredictor(
                sensory_dim, embed_dim
            )
    ...
    def forward(
        ...
        if self.use_sensory_aux:
            ...
            aux_output['sensory_logits'] = (
                self._aggregate_with_selector(
                    logits, selector
                )
            )
