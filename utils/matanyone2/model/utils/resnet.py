"""
resnet.py - Modified ResNet structure with extra channels.
"""
...
def load_weights_add_extra_dim(
    target, source_state, extra_dim=1
):
...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = conv3x3(
            planes, planes, stride=1, dilation=dilation
        )
...
class ResNet(nn.Module):
    ...
    def _make_layer(
        ...
        for i in range(1, blocks):
            layers.append(
                block(
                    self.inplanes, planes, dilation=dilation
                )
            )
