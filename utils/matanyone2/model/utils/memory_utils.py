...
def get_similarity(
    ...
    ms = (
        ms.flatten(start_dim=1).unsqueeze(2)
        if ms is not None
        else None
    )
    qk = qk.flatten(start_dim=2)
    qe = (
        qe.flatten(start_dim=2) if qe is not None else None
    )
...
def get_affinity(
    ...
    similarity = get_similarity(
        mk, ms, qk, qe, uncert_mask=uncert_mask
    )
...
def readout(
    ...
    if uncert_mask is not None:
        uncert_mask = uncert_mask.flatten(start_dim=2).expand(
            -1, CV, -1
        )
        mem = mem * uncert_mask
