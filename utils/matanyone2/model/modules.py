...
class UpsampleBlock(nn.Module):
    def __init__(
        self, in_dim: int, out_dim: int, scale_factor: int = 2
    ):
        super().__init__()
    ...
    def forward(
        self, in_g: torch.Tensor, skip_f: torch.Tensor
    ) -> torch.Tensor:
        g = F.interpolate(
            in_g,
...
class DecoderFeatureProcessor(nn.Module):
    def __init__(
        self, decoder_dims: List[int], out_dims: List[int]
    ):
        super().__init__()
    ...
    def forward(
        ...
        outputs = [
            func(x)
            for x, func in zip(
                multi_scale_features, self.transforms
            )
        ]
        return outputs
...
def _recurrent_update(
    h: torch.Tensor, values: torch.Tensor
) -> torch.Tensor:
    ...
    new_h = (
        forget_gate * h * (1 - update_gate)
        + update_gate * new_value
    )
    return new_h
...
class SensoryUpdater_fullscale(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.g16_conv = GConv2d(
            g_dims[0], mid_dim, kernel_size=1
        )
        self.g8_conv = GConv2d(
            g_dims[1], mid_dim, kernel_size=1
        )
        self.g4_conv = GConv2d(
            g_dims[2], mid_dim, kernel_size=1
        )
        self.g2_conv = GConv2d(
            g_dims[3], mid_dim, kernel_size=1
        )
        self.g1_conv = GConv2d(
            g_dims[4], mid_dim, kernel_size=1
        )
    ...
    def forward(
        self, g: torch.Tensor, h: torch.Tensor
    ) -> torch.Tensor:
        g = (
            self.g16_conv(g[0])
            + self.g8_conv(
                downsample_groups(g[1], ratio=1 / 2)
            )
            + self.g4_conv(
                downsample_groups(g[2], ratio=1 / 4)
            )
            + self.g2_conv(
                downsample_groups(g[3], ratio=1 / 8)
            )
            + self.g1_conv(
                downsample_groups(g[4], ratio=1 / 16)
            )
        )
