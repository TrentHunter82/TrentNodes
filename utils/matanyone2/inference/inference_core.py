"""
InferenceCore for MatAnyone 2.

Vendored from https://github.com/pq-yang/MatAnyone2
with imports fixed for use as a vendored module and
video I/O methods removed (not needed in ComfyUI).
"""
...
class InferenceCore:
    ...
    def __init__(
        ...
        *,
        ...
        if stagger_updates >= self.mem_every:
            self.stagger_ti = set(
                range(1, self.mem_every + 1)
            )
        else:
            self.stagger_ti = set(
                np.round(
                    np.linspace(
                        1, self.mem_every, stagger_updates
                    )
                ).astype(int)
            )
    ...
    def _add_memory(
        ...
        *,
        ...
        msk_value, sensory, obj_value, _ = (
            self.network.encode_mask(
                image,
                pix_feat,
    ...
    def _segment(
        ...
        memory_readout = (
            self.object_manager.realize_dict(
                memory_readout
            )
        )

        sensory, _, pred_prob_with_bg = (
    ...
    def step(
        ...
        *,
        ...
        image: 3*H*W in [0, 1]
        mask: H*W in [0, 255] (matting=True) for init
        objects: list of object ids in mask
        first_frame_pred: reset memory and re-run
            warmup on this frame
        """
        ...
        if self.max_internal_size > 0:
            ...
            if min_side > self.max_internal_size:
                resize_needed = True
                new_h = int(
                    h / min_side * self.max_internal_size
                )
                new_w = int(
                    w / min_side * self.max_internal_size
                )
                image = F.interpolate(
                    image.unsqueeze(0),
                    size=(new_h, new_w),
        ...
        ms_feat, pix_feat = (
            self.image_feature_store.get_features(
                self.curr_ti, image
            )
        )
        key, shrinkage, selection = (
            self.image_feature_store.get_key(
                self.curr_ti, image
            )
        )
        ...
        if mask is not None:
            corresponding_tmp_ids, _ = (
                self.object_manager.add_new_objects(objects)
            )
            ...
            if need_segment:
                ...
                new_masks = []
                for mask_id, tmp_id in enumerate(
                    corresponding_tmp_ids
                ):
                    if idx_mask:
                        this_mask = (
                            mask == objects[mask_id]
                        ).type_as(pred_prob_no_bg)
                    else:
                        this_mask = mask[tmp_id]
                    if tmp_id > pred_prob_no_bg.shape[0]:
                        new_masks.append(
                            this_mask.unsqueeze(0)
                        )
                    else:
                        pred_prob_no_bg[tmp_id - 1] = (
                            this_mask
                        )
                mask = torch.cat(
                    [pred_prob_no_bg, *new_masks], dim=0
                )
            elif idx_mask:
                if len(objects) == 0:
                    if delete_buffer:
                        self.image_feature_store.delete(
                            self.curr_ti
                        )
                    return torch.zeros(
                        (
            ...
            if matting:
                ...
                pred_prob_with_bg = torch.cat(
                    [1 - mask, mask], 0
                )
            else:
                pred_prob_with_bg = aggregate(mask, dim=0)
                pred_prob_with_bg = torch.softmax(
                    pred_prob_with_bg, dim=0
                )
    ...
    def output_prob_to_mask(
        ...
        if matting:
            ...
            for tmp_id, obj in (
                self.object_manager.tmp_id_to_obj.items()
            ):
