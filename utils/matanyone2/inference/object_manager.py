...
class ObjectManager:
    """
    Object IDs are immutable. Temporary IDs are positions
    in the tensor (start from 1).
    """
    ...
    def _recompute_obj_id_to_obj_mapping(self) -> None:
        self.obj_id_to_obj = {
            obj.id: obj for obj in self.obj_to_tmp_id
        }

    def add_new_objects(
        self,
        objects: Union[
            List[ObjectInfo], ObjectInfo, List[int]
        ],
    ) -> (List[int], List[int]):
        ...
        for obj in objects:
            ...
            if obj in self.obj_to_tmp_id:
                corresponding_tmp_ids.append(
                    self.obj_to_tmp_id[obj]
                )
                ...
                self.all_historical_object_ids.append(
                    new_obj.id
                )
    ...
    def delete_objects(
        self, obj_ids_to_remove: Union[int, List[int]]
    ) -> None:
    ...
    def purge_except(self, obj_keep_idx: List[int]) -> None:
        obj_keep_idx_set = set(obj_keep_idx)
        to_delete = [
            obj.id
            for obj in self.obj_to_tmp_id
            if obj.id not in obj_keep_idx_set
        ]
        if to_delete:
