...
class KeyValueMemoryStore:
    ...
    def add(
        ...
        assert not self.save_selection or len(
            selection.shape
        ) == 3
        ...
        if supposed_bucket_id >= 0:
            ...
            bucket_exist = (
                supposed_bucket_id in self.buckets
            )
            for obj, value in values.items():
                if bucket_exist:
                    ...
                    assert (
                        obj
                        in self.buckets[supposed_bucket_id]
                    )
                    _add_last_dim(
                        self.v,
                        obj,
                    ...
                    self.v[obj] = value
            self.buckets[supposed_bucket_id] = list(
                values.keys()
            )
        else:
            new_bucket_id = None
            ...
            for obj, value in values.items():
                ...
                if obj in self.v:
                    ...
                    bucket_used = [
                        bid
                        for bid, oids in self.buckets.items()
                        if obj in oids
                    ]
        ...
        for bucket_id in self.buckets:
            ...
            _add_last_dim(
                ...
                bucket_id,
                key,
                prepend=add_as_permanent[bucket_id],
            )
            _add_last_dim(
                self.s,
                bucket_id,
    ...
    def update_bucket_usage(
        self, bucket_id: int, usage: torch.Tensor
    ) -> None:
        ...
        if usage.shape[-1] == 0:
            return
        self.use_cnt[bucket_id] += usage.view_as(
            self.use_cnt[bucket_id]
        )
    ...
    def sieve_by_range(
        ...
        bucket_num_elements = (
            self.k[bucket_id].shape[-1]
            - self.perm_end_pt[bucket_id]
        )
        ...
        self.k[bucket_id] = torch.cat(
            [k[:, :, :start], k[:, :, end:]], -1
        )
        self.s[bucket_id] = torch.cat(
            [s[:, :, :start], s[:, :, end:]], -1
        )
        if self.save_selection:
        ...
        for obj_id in object_ids:
            v = self.v[obj_id]
            self.v[obj_id] = torch.cat(
                [v[:, :, :start], v[:, :, end:]], -1
            )

    def remove_old_memory(
        self, bucket_id: int, max_len: int
    ) -> None:
        self.sieve_by_range(bucket_id, 0, -max_len, max_len)

    def remove_obsolete_features(
        self, bucket_id: int, max_size: int
    ) -> None:
        object_ids = self.buckets[bucket_id]
        ...
        for bi in range(bs):
            ...
            assert (
                survived.shape[-1] == survivals[0].shape[-1]
            )
