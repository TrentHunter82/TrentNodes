...
class ImageFeatureStore:
    """Cache for image features to avoid recomputation."""

    def __init__(self, network, no_warning: bool = False):
    ...
    def _encode_feature(
        ...
        ms_features, pix_feat = self.network.encode_image(
            image, last_feats=last_feats
        )
        key, shrinkage, selection = (
            self.network.transform_key(ms_features[0])
        )
        self._store[index] = (
            ms_features,
            pix_feat,
    ...
    def __del__(self):
        if len(self._store) > 0 and not self.no_warning:
            warnings.warn(
                f'Leaking {self._store.keys()} in the'
                f' image feature store'
            )
